...
First, explain the scientific problem and the data set.  Ideally you can do this in no more than one or two slides -- perhaps one about the problem and one about the data, although you're the best judge of the right balance here.  Explaining the problem or the data set would be a good use of a bulleted list, so that people can read some key facts about the problem/data as they listen to you describe stuff.  You might also/instead show a picture of the data, maybe with those key facts listed beside it or underneath it.  On occassion you might need more slides than 1-2 slides, but only if the problem is really complicated _and_ those complications are essential for understanding what comes next.  The key thing is to convey a broadly accessible summary of the problem.  Make it interesting, but keep it simple.  

Second, you should show the results from applying an off-the-shelf, existing method to this data.  If you can do this in one picture, great.  This is your "straw man": the thing your method should beat.  Focus on intuition, not on details.    

Third and finally, you should explain what's wrong/suboptimal with the off-the-shelf method.  This tells the audience why they should pay attention when you introduce your new method.  Again, try to do this in one slide at most.  Or maybe don't even use a slide, if you can just explain the problems as you show a picture of what you get when you fit the method to the data.   
